define(['core', 'tpl', 'biz/goods/cart', 'biz/plugin/diyform'], function(core, tpl, cart, diyform) {
    ...
    modal.open = function(params) {
        ...
        if (modal.goodsid != params.goodsid) {
            ...
            modal.categoryid = params.categoryid;
            var obj = {
                id: params.goodsid
            };
            if (params.liveid) {
    ...
    modal.init = function() {
		...
		// $('.categoryconfirmbtn', modal.container.container).unbind('click').click(function() {
  //           console.log($('.option-picker .diyform-container').length);
  //           if ($('.option-picker .diyform-container').length > 0) {
  //               var diyformdata = diyform.getData('.option-picker .diyform-container');
  //               var categorystatus = 1;

  //                 console.log(diyformdata);
  //               if (!diyformdata) {
  //                   return
  //               } else {
  //                    core.json('order/create/categorydiyform', {                        
  ...
  //                       optionid:modal.params.optionid,
  //                   }, function(ret) {
  //                      location.href = core.getUrl('goods/package/detail', {
  //                           goodsid: modal.goodsid,
  //                           categoryid:modal.categoryid,
  //                           pid:ret.result.pid,                            
  //                       })
  ...
  //       } 
        $('.categoryconfirmbtn', modal.container.container).unbind('click').click(function() {
            ...
            var diyformdata = diyform.getData('.diyform-container');
            var categorystatus = 1;
            var diyformdata= JSON.stringify(diyformdata);
            console.log(diyformdata);
            core.json('order/create/categorydiyform', {                        
                    goodsid: modal.goodsid,
                    pid:modal.pid,
    ...
    modal.show = function() {
        ...
        if (modal.needlogin) {            
            if(modal.params.categoryid){
                var backurl = core.getUrl('goods/package/detail', {
                    pid: modal.params.pid
                });
            }else{
    ...
    modal.changeCartcount = function(count) {
        ...
        // modal.params.onConfirm(modal.params.total, modal.params.optionid, modal.params.titles)
    };
    return modal
});