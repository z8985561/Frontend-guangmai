define(['core', 'tpl'], function(core, tpl) {
		...
		$('.pay-btn').unbind('click').click(function() {
			if(modal.params.addressid==undefined){
                 ...
                 if($(this).data('addressid')){
                    modal.params.addressid = $(this).data('addressid');
                 }else{
                    FoxUI.toast.show('请添加收货地址');
                    return ;
                 }
            }else{
                modal.params.addressid = modal.params.addressid;
            }
			var btn = $(this);
			modal.pay(btn)
		});
	...
	modal.pay = function(btn) {
		...
		if (type == 'wechat') {
			...
			return
		} else if (type == 'qukuailian') {
			FoxUI.confirm('确认要支付吗?', '提醒', function() {
				location.href = core.getUrl('order/pay/returnurl', {
				orderid: modal.params.orderid,
				ordersn: modal.params.ordersn,
				money: modal.params.money,
				type: 0,
				})
			}, function() {
				btn.removeAttr('stop')
	...
	modal.complete = function(btn, type) {
		...
		setTimeout(function() {
			core.json('member/myyuncang/pay', {
				id: modal.params.orderid,
				ordersn: modal.params.ordersn,
				type: type,
